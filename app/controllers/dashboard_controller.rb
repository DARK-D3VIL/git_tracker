class DashboardController < ApplicationController
  ...
  def index
    ...
    time_range = nil
    if dashboard_params[:range].present?
      if dashboard_params[:range] == "7days"
        time_range = Time.current - 7.days 
      elsif dashboard_params[:range] == "30days"
        time_range = Time.current - 30.days
      end
    end

    status = "closed"
    if dashboard_params[:status].present? && dashboard_params[:status] == "all"
      status = dashboard_params[:status]
    end

    sort_by = "dev_score"
    if dashboard_params[:sort_by].present? && dashboard_params[:sort_by] == 'rev_score'
      sort_by = dashboard_params[:sort_by]
    end
    ...
    if dashboard_params[:query].present?
      query = dashboard_params[:query].downcase
